...
if [[ ${PV} == "999999" ]] ; then
	...
	# re bundle: I bundled it myself (I don’t trust tamiko’s bundle enough)
	SRC_URI="
		!binary? (
			https://github.com/tianocore/edk2/archive/edk2-stable${PV}.tar.gz -> ${P}.tar.gz
			http://uploaduji.mujmalysvet.cz/${P}-bundled.tar.xz
		)
		"
...
src_compile() {
	...
	# XXX: this causes a hang on some futex; qemu seems to work fine even without it
